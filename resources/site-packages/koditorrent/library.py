...
def library_add(content_type):
    ...
    import xbmc
    import xbmcgui
    from koditorrent.magnet import ensure_magnet, display_name
    from koditorrent.utils import get_show_info_from_name
    from urllib import quote_plus, unquote

    play_url = "plugin://plugin.video.koditorrent/play/"
...
def library_install():
    ...
    _rescan_library()
    xbmcgui.Dialog().ok("koditorrent", "Installation complete.", "")
...
def library_uninstall():
    ...
    with closing(sqlite3.connect(_get_video_db())) as conn:
        ...
        for content_type, entry in LIBRARY_PATHS.items():
            ...
            _rescan_library(entry["strPath"])
        root.write(sources_filename)
    xbmcgui.Dialog().ok("koditorrent", "Uninstallation complete.", "")
