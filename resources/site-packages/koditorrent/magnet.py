from koditorrent.plugin import plugin
import xbmc,xbmcaddon,xbmcgui
from koditorrent import subtitles
...
def display_name(magnet_uri):
    import urlparse
    from koditorrent.utils import first
...
def play(uri):
	from koditorrent.player import TorrentPlayer
	subtitle = ''
	try:
		season=''
		episode=''
		tagname = xbmc.getInfoLabel("ListItem.Label")
		try: imdb = xbmc.getInfoLabel("ListItem.MPAA")
		except: imdb = ''
		try: name = xbmc.getInfoLabel("ListItem.OriginalTitle")
		except: 
			try: name = xbmc.getInfoLabel("ListItem.Title")
			except: name = xbmc.getInfoLabel("ListItem.Label")
		if xbmc.getInfoLabel("ListItem.TVShowTitle") <> '':
				tagname = xbmc.getInfoLabel("ListItem.TVShowTitle")
		...
		if getSetting("subtitles") == 'true': subtitle = subtitles.getsubtitles(name,tagname,season,episode,getSetting("sublang1"),getSetting("sublang2"),imdb)
	except: pass
	...
	TorrentPlayer().init(uri,subtitle).loop()
