...
import time
import urlparse
from koditorrent import plugin, torrent2http
from koditorrent.common import RESOURCES_PATH
from koditorrent.platform import PLATFORM
from koditorrent.ga import track_event
from koditorrent.utils import url_get_json
...
class TorrentPlayer(xbmc.Player):
    ...
    def _get_status_lines(self, status):
        extension = self.getFileExtension(status["name"])
        message = self.getExtensionMessage(extension)
        if message == "":
            message = status["name"]
        return [
            message,
        ...
        with closing(torrent2http.start(**self.torrent2http_options)) as t2h_instance:
            ...
            with closing(SafeDialogProgress(delay_create=0)) as dialog:
                ...
                while not has_resolved:
                    ...
                    BUFFERSIZE = self.getBufferSize(extension)
